...
#PLEASE DO NOT EDIT ANYTHING BELOW THIS LINE UNLESS YOU'RE ABSOLUTELY SURE WHAT YOU'RE DOING
...
function todays_count($postnum, $unique) {
	...
	$excludepost = $EXCLUDE ? excheck($postnumb) : 0;
...
function todays_count_widget($views,$list) {
	...
	$todays_widget = $lbeg."\n";
	$and = $EXCLUDE ? excql() : "";
	$widgets = $wpdb->get_results("SELECT * from $table_name inner join $table_posts on $table_posts.ID=$table_name.postnum WHERE time = '$nowisnow' and postcount > 0 and post_status = 'publish' $and ORDER BY postcount DESC LIMIT $limit");
  ...
  $todays_widget_temp .= "<li><a href=\"$home_url/?p=$id_post\" title=\"$text\">$title_post</a> ($postcount$views)</li>\n";
	...
	$todays_widget_temp .= "<li>No posts viewed yet. $and</li>";
	}
...
function todays_overall_main($postnum, $visited, $times, $sofar) {
  ...
  global $EXCLUDE;
	...
	$excludepost = $EXCLUDE ? excheck($postnum) : 0;
	if ($excludepost != 1) {
	$nowisnow = todays_date();
	$show_overall_main= "";
...
function todays_overall_count_widget($views,$list) {
	...
	$widgets = $wpdb->get_results("SELECT * from $table_name_all inner join $table_posts on $table_posts.ID=$table_name_all.postnum WHERE postcount > 0 and post_status = 'publish' $and ORDER BY postcount DESC LIMIT $limit");
...
function admin_rightnow() {
	...
	$widgets = $wpdb->get_results("SELECT * FROM $table_name inner join $table_posts on $table_posts.ID=$table_name.postnum WHERE time = '$nowisnow' and postcount > 0 and post_status = 'publish' $and ORDER BY postcount DESC LIMIT 1");
	$widgets_num = $wpdb->get_var("SELECT COUNT(*) FROM $table_name inner join $table_posts on $table_posts.ID=$table_name.postnum WHERE time = '$nowisnow' and postcount > 0 and post_status = 'publish'");
	if ($widgets_num) {
