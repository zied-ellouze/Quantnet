...
<script type="text/javascript" src="<?php bloginfo('template_url'); ?>/js/jquery-1.5.2.js"></script>
		...
		if(type=="rate"){
			...
			$('#reviewdata').html("");
			$("#loadingContainer").html("<img src='<?php bloginfo('template_url'); ?>/images/loading.gif />");
			//alert($("#loadingContainer").html());
			getReviews($('#post_val').val(),$('#order_type').val(),$('#sort_type').val())
			$("#loadingContainer").html("");
		}
		
		if(type=="name"){
			$('#sort_type').val("post_title");
			if($('#order_type').val()=="asc"){
				$('#order_type').val("desc");
			}else{
				$('#order_type').val("asc");
		...
		$(document).ready(function(){
				...
				getReviews($('#post_val').val(),"asc","post_title");	
		...
		function getReviews(str,type,sort_name){
			$.post("<?php bloginfo('template_url'); ?>/review_response.php",{data:str, order:type, orderby:sort_name},function(obj) {
			$.each(obj.items, function(index, value) { 
			$.each(obj.items[index], function(index1, value1) {
			var tblRow ="<br /><div><strong>"+index1+"</strong> :   "+value1+"</div><br />";			
			$(tblRow).appendTo("#reviewdata");
			 });
		});
	}, "json");
	...
	<?php if ( ( function_exists( 'get_post_format' ) && 'gallery' == get_post_format( $post->ID ) ) || in_category( _x( 'gallery', 'gallery category slug', 'twentyten' ) ) ) : ?>
		<div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
         ...
         elseif ( ( function_exists( 'get_post_format' ) && 'aside' == get_post_format( $post->ID ) ) || in_category( _x( 'asides', 'asides category slug', 'twentyten' ) )  ) : ?>
			...
			//echo "<pre>"; print_r($servrs);	 echo "</pre>";
			if($servrs[0] == '') //for getting all the posts other than that of job posts
                   {
...
src="http://platform.twitter.com/widgets/tweet_button.html?url=<?php the_permalink()?>&amp;via=quantnet&amp;text=<?php the_title();?>&amp;count=horizontal" width="106" height="21"></iframe></div>
						...
						<input type="hidden" name="post_val" id="post_val" value="<?php echo $postID; ?>" />
						<input type="hidden" name="sort_type" id="sort_type" value="post_title" />
						<input type="hidden" name="order_type" id="order_type" value="asc" />
						
						
<div>Sort By  <a href="javascript://" id="review_sort" onclick="sort_reviews('rate');"><strong>Rate</strong></a>&nbsp;&nbsp;&nbsp;<a href="javascript://" id="name_sort" onclick="sort_reviews('name');" ><strong>Name</strong></a>&nbsp;&nbsp;&nbsp;<a href="javascript://" id="program_type_sort" onclick="sort_reviews('program');"><strong>Program Type</strong></a></div>	

						
<!--						<strong>All Reviews</strong>
-->						<!--<div><strong>Sort By :</strong><a href="javascript://" id="rate">Rating</a>&nbsp;&nbsp;&nbsp;<a href="javascript://" id="name">Name</a>&nbsp;&nbsp;<a href="javascript://" id="program_type">Program Type</a></div>-->
		 ...
		 <div id="post-<?php the_ID(); ?>" <?php post_class(); ?>>
			<h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php printf( esc_attr__( 'Permalink to %s', 'twentyten' ), the_title_attribute( 'echo=0' ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2>
                        <?php if ( is_archive() || is_search() || is_home() ) : // Only display excerpts for archives and search. ?>
                            <div class="entry-summary">
                                    <div class="featured-img"><?php the_post_thumbnail(); //for showing thumbnail ?></div><?php the_content(); ?><?php ngg_excerpt(); ?>
                            </div><!-- .entry-summary -->
                        <?php else : ?>
			<div class="entry-content">
				<?php the_content( __('Continue reading <span class="meta-nav">&rarr;</span>', 'twentyten' ) ); ?>
                        ...
                        <?php if(!is_home() ) { ?>
                            <div class="tag-links">
                                <?php
                                    ...
                                    if ( $tags_list ):
                                ?>
                                    <?php printf( __( '<span class="%1$s">Tags:</span> %2$s', 'twenty ten' ), 'entry-utility-prep entry-utility-prep-tag-links', $tags_list ); ?>
                                <?php endif; ?>
                            </div>
                        <?php //if(function_exists('the_views')) { the_views(); } ?> <?php } ?>
                        <?php edit_post_link( __( 'Edit', 'twenty ten' ), '<span class="edit-link">', '</span>' ); ?>
                        <?php if(!is_page()) { ?>
                            <div class="post-meta">
                                <div class="post-comment">
                                    <span>
            ...
            <?php if (  $wp_query->max_num_pages > 1 ) : ?>
                    <div id="nav-below" class="navigation">
                            <div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> Older posts', 'twenty ten' ) ); ?></div>
                            <div class="nav-next"><?php previous_posts_link( __( 'Newer posts <span class="meta-nav">&rarr;</span>', 'twenty ten' ) ); ?></div>
                    </div><!-- #nav-below -->
