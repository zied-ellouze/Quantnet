...
if(!function_exists('quant_get_all_program_images')):
	function quant_get_all_program_images($program_id = null, $limit = 4){
		global $wpdb;
		if($program_id == null):
			return array();
		endif;
		...
		$review_images = $wpdb->get_results("SELECT t1.id, t1.post_id, t2.lead_id, t2.field_number, t2.value FROM wp_rg_lead as t1, wp_rg_lead_detail as t2, wp_posts as t3 WHERE t3.post_parent = '".$program_id."' AND t3.ID = t1.post_id AND t1.id = t2.lead_id AND (t2.field_number = 32 || t2.field_number = 33 || t2.field_number = 34 || t2.field_number = 35 || t2.field_number = 36 || t2.field_number = 37 || t2.field_number = 38 || t2.field_number = 39 || t2.field_number = 40) LIMIT ".$limit);
		$arr_images = array();
		...
		$total_images = array_merge($images,$arr_images);
		
		if(count($total_images) > $limit):
			$total_images = array_slice($total_images, 0, ($limit-1));
		endif;
